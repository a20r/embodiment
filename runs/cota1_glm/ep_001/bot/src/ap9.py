...
try:
    _h=f(get('d3'),0.0)
    th=math.radians(_h)
    log(f'FRESH FRAME: th init from compass hd={_h:.1f}')
except Exception as e:
    log('FRAME INIT FAIL '+str(e))
AX,AY,ATH=41.45,107.65,161.4     # crossing anchor
SA=[-12.5,-0.5]                  # start-area junction
EXTL=[13.0,1.0]                  # inner-loop exit target
...
stuck_since=None; prev={'lap':None,'goal':None}
...
while True:
    try:
        ...
        if t<fw_until:
            ...
            if front<0.50:
                rev_until=t+1.0; rev_start=t; uturn_until=0; emit(-45,0); mode='TOREV-UT'
            else:
                emit(24,85); mode='UTURN'
        elif t<exit_until:
            if front<0.40:
            ...
            if front<0.40:
                ...
                if os.path.exists('/bot/src/ZSEEK'):
                    ...
                    if dmin[0]<6.0:
                        ...
                        homing=False
                        log(f'HOMING reached ring d={dmin[0]:.1f}')
                    else:
                        wx,wy=RING[dmin[1]]
                        errh=math.degrees(math.atan2(wy-py,wx-px)-th)
                        errh=(errh+180)%360-180
                        steer=clamp_s(1.5*errh+25*(avgL-avgR)-20*wz)
                        d7c=max(-85,min(85,steer))
                        d4=30 if front>0.9 else 16; mode='HOME'
                        emit(d4,d7c)
                if not homing and math.hypot(px-JT[0],py-JT[1])<12.0 and t-jt_t>30:
                    ...
                    emit(d4,d7c)
        if mode in ('?','TOREV','TOREV-UT','TOREV-EXIT','WAIT'): emit(d4,d7c)
        csvn+=1
        if csvn%4==0:
            CSV.write(f'{t-t0:.2f},{tick},{px:.3f},{py:.3f},{math.degrees(th)%360:.1f},{hd:.1f},{sp:.3f},{wz:.3f},{d4},{d7c},{mode},{front:.2f},{avgL:.2f},{avgR:.2f},{vt:.2f}\n')
        cyc+=1
        if cyc%400==0: log(f'c{cyc} hd={hd:.0f} v={sp:.2f} vt={vt:.2f} pose=({px:.1f},{py:.1f}) front={front:.2f} {mode} {d8}')
        if t-hb>2:
            hb=t
