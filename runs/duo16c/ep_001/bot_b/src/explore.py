...
c=0; t0=time.time(); last_tx=0; last_ping=0; d0_hits=[]; hold=0; unstick_t=time.time(); unstick_x=x; unstick_y=y
bias=0.0; bias_t=0
stop()
time.sleep(0.2)
while time.time()-t0<390:
    ...
    # radio: aggressive when d5=1 (other robot maybe in range)
    txp=1.5 if d5v=='1' else 4.0
    if now-last_tx>txp:
        wr('d8','A1 x=%.1f y=%.1f H=%.0f d5=%s'%(x,y,H,d5v)); last_tx=now
    if d5v=='1' and c%3==0:
        rx=rd('d10','0.12')
        if rx: log(ev='RX',msg=rx); print('RX:',rx,flush=True)
    # d5 pause-and-ping every 30s
    if d5v=='1' and now-last_ping>30:
        last_ping=now; stop()
        for _ in range(8):
            wr('d8','A1 PING x=%.1f y=%.1f'%(x,y)); time.sleep(0.12)
            rx=rd('d10','0.18')
            if rx: log(ev='RX',msg=rx); print('RX:',rx,flush=True)
        time.sleep(0.5)
    # d0 handling
