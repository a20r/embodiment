...
def recover():
    L('RECOVER at t=%.0f'%(time.time()-t0))
...
try:
  while True:
    ...
    if now-lastlog>1.0:
        L('t=%.0f pos %.2f,%.2f h=%.0f f=%.2f r=%.2f l=%.2f odo=%d lid=%s'%(now-t0,x,y,h or -1,front,right,left,lasto,','.join('%.2f'%v for v in l)))
        lastlog=now
    if now-lasttx>10:
    ...
    if fl<0.14 and front<0.5:
        b.wheels(30,-30); time.sleep(0.15); continue
    if fr<0.14 and front<0.5:
        b.wheels(-30,30); time.sleep(0.15); continue
    if front<0.32 or (front<0.45 and fr<0.22 and fl<0.22):
        b.wheels(-35,35); time.sleep(0.12); continue
    if right>0.55:
        b.wheels(48,20); time.sleep(0.12); continue
    err=(0.26-right)
