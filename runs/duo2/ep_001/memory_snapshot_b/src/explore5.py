...
from robolib2 import Bot, angdiff, poll

b=Bot()
log=open('/memory/telemetry.log','a',buffering=1)
def L(*a): log.write(' '.join(str(x) for x in a)+'\n')
L('=== ex5 start',time.time())

MT=0.00176
x,y=0.0,0.0
def enc():
    try: return int(poll('d8') or 0)
    except: return 0
lasto=enc()
t0=time.time()
...
try:
  while True:
    ...
    # cells and loop detection
    if now-lastcell>1.0:
        lastcell=now
        c=(round(x/0.6),round(y/0.6))
        cells[c]=cells.get(c,0)+1
        if mode=='follow' and cells[c]>7:
            # dash: aim at most open beam
            i=max(range(16),key=lambda j:l[j])
            dash_hdg=((h or 0)+i*22.5)%360
            mode='dash'; dash_until=now+14
            if random.random()<0.4: hand=-hand
            cells={}
            L('DASH t=%.0f pos %.2f,%.2f hdg %.0f hand %d'%(now-t0,x,y,dash_hdg,hand))
    front=l[0]
    ...
    if hand<0:
        ...
        fl,fr=fr,fl
    if now-lastlog>1.0:
        L('t=%.0f pos %.2f,%.2f h=%.0f f=%.2f m=%s%d odo=%d lid=%s'%(now-t0,x,y,h or -1,front,mode[0],hand,lasto,','.join('%.2f'%v for v in l)))
        lastlog=now
    if now-lasttx>10:
    ...
    if mode=='dash':
        if now>dash_until:
            mode='follow'; L('DASH end pos %.2f,%.2f'%(x,y))
        elif front<0.35:
            mode='follow'; L('DASH wall pos %.2f,%.2f'%(x,y))
        else:
            d=angdiff(dash_hdg,h or 0)
            turn=max(-25,min(25,d*1.2))
            sp=60 if front>0.9 else 35
