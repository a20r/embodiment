import os
BR=float(os.environ.get("BR","17"))
import os
TX=float(os.environ.get('TX','-1.3'))
TY=float(os.environ.get('TY','0.0'))
...
import json as _j
try:
    _p=_j.load(open('/memory/pose.json'))
    odo.x=_p['x']; odo.y=_p['y']
...
def upd_grad():
    import math as _m
    try: _b=float(open('/memory/br.txt').read())
    except Exception: _b=BR
    grad[0]=_m.cos(_m.radians(_b)); grad[1]=_m.sin(_m.radians(_b)); grad[2]=True; return
    n=len(d5hist)
...
def housekeep(L,note=None):
    ...
    if st.get('here') or st.get('goal'):
        speed(0,0); tx('AT_GOAL'); log(note='HERE',st=st,x=odo.x,y=odo.y)
        return True
    if now-state['tx']>2:
...
def choose(L,h):
    ...
    for i in range(16):
        ...
        sc=min(d,2.0)-0.55*min(v,10)-min(i,16-i)*0.04+random.uniform(0,0.05)
        if grad[2]:
            gm=math.hypot(grad[0],grad[1])
            if gm>1e-6:
                sc+=0.9*(math.cos(th)*grad[0]+math.sin(th)*grad[1])/gm
