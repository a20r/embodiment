...
def goto(tx,ty,maxt=5.0):
    ...
    while time.time()-t0<maxt:
        ...
        if math.hypot(tx-rx[0],ty-rx[1])<0.12:
...
# dead-reckoned local frame at sweep start
rx=[0.0,0.0]; prev=None
def odo():
    global prev
    e=r.enc(); h=r.heading()
    if e and prev and h is not None:
...
while time.time()-t0<600:
    st=r.status() or (0,0,0)
    d0=r.read(0,0.03); d5=r.read(5,0.03)
    if st[2]==1:
        ...
        while True:
            ...
            time.sleep(1.0)
    print(f"sweep ring={ring} d0={d0} d5={d5} st={st}", flush=True)
    # spiral step
    ang+=55
    if ring>=0 and ang%360==0: pass
    tgt=0.28+0.07*ring
    turnto(ang)
    res=goto(rx[0]+tgt*math.cos(math.radians(ang)), rx[1]+tgt*math.sin(math.radians(ang)))
    odo()
    if res=='wall':
        ring+=0
    if res=='HERE':
        continue
    # expand ring slowly
    if (int(ang/55))%8==0: ring=min(ring+0.34, 2.0)
print("sweep ended", flush=True)
