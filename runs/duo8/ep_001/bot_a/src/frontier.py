import time, math, threading, json, random, os
...
LOG=open('/memory/run6.log','a',buffering=1)
...
def upd_pose(h,skip):
    try:
        L=float(rline(7)); R=float(rline(8))
    except: return
...
def update_map(h,l):
    ...
    for k in range(16):
        ...
        for i in range(1,n):
            ...
            free.add(fc); occ.discard(fc)
        if r<2.4:
            oc=cellof(x+c*r,y+s*r)
            occ.add(oc); free.discard(oc)
...
def plan():
    start_c=cellof(*pose)
    b=blocked_set()
    # ensure start passable: find nearest free non-blocked cell to start
    if start_c in b or start_c not in free:
        q=deque([start_c]); seen={start_c}; found=None
        while q:
            c=q.popleft()
            if c in free and c not in b: found=c; break
            if abs(c[0]-start_c[0])>8 or abs(c[1]-start_c[1])>8: continue
    ...
    while q:
        c=q.popleft()
        # frontier? free cell with an unknown 4-neighbor
        for n2 in neighbors(c):
            if n2 not in free and n2 not in occ:
...
def compass_err(tgt_h,h):
    return (tgt_h-h+180)%360-180

start=time.time()
last_tx=0; last_stat=0; last_plan=0; last_save=time.time()
path=None
hist=[]
mode="explore"
log("START frontier")
...
try:
  while time.time()-start<4800:
    ...
    if path:
        ...
        dx=wx-pose[0]; dy=wy-pose[1]
        dist=math.hypot(dx,dy)
        if dist<0.1 and wi==len(path)-1:
            path=None; motors(0,0)
        else:
            tgt_h=(-math.degrees(math.atan2(dy,dx)))%360
            err=compass_err(tgt_h,h)
            if abs(err)>50:
                v=max(min(err*0.8,35),-35); v=math.copysign(max(abs(v),6),v)
                motors(v,-v)
            else:
                fr=min(l[0],l[1],l[15])
                base=80 if fr>0.7 else 50
                st=max(min(err*1.5,30),-30)
                motors(base+st,base-st)
    else:
        motors(20,-20)  # spin to find frontier
    if t-last_tx>2:
        last_tx=t
    ...
    if t-last_stat>3:
        ...
        log("POS",round(pose[0],2),round(pose[1],2),"h",round(h,1),"d5",d5,"st",st6,"pl",len(path) if path else 0,"occ",len(occ))
