...
def S(tag, **kw):
    ...
    json.dump(log, open('/memory/ep8.json','w'))
    return e
...
while time.time()-t0 < 12000:
    ...
    if e0.get('d5')=='1':
        print('D5 B-NEAR: stop+ready',flush=True)
        d.set(0,0)
        for k in range(8):
            tx('READY A HERE SEE d11=%.2f'%e0['d11']); time.sleep(3)
            if rd('d5')=='0': break
        continue
    ...
    if e1['d11']>0.92:
        print('CONTACT post-leg',flush=True); continue
    dd=e1['d11']-e0['d11']
