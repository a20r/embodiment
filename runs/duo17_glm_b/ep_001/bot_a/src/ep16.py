...
def S(tag, **kw):
    ...
    json.dump(log, open('/memory/ep16.json','w'))
    return e
...
while time.time()-t0 < 12000:
    ...
    right = avg(l,(4,5,6)); left = avg(l,(10,11,12))
    cur = e0['d11']
    if cur>=GREEDY:
        # greedy mode: follow d11 rises
        ...
        e1=S('leg%d-b'%n)
        dd=e1['d11']-e0['d11']
        print('GREEDY dd=%+.3f'%(dd,),flush=True)
        if dd<-0.012: turn(60); S('turn60')
        continue
    # wall-follow: keep RIGHT hand on wall
    if front>0.35:
        if right>0.9 and front>0.5:
            # opening on right: take it (turn right)
            turn(50)
        drive(14)
    else:
        # blocked: turn LEFT (keep right wall)
        turn(85)
        drive(8)
