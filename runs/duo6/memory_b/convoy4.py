...
while True:
  try:
    ...
    try: wp_i
        ...
        wps=[(10,-20),(15,20),(-10,-25),(35,0),(-20,-20),(0,35)]
        wp_i=0; wp_t=now
    if math.hypot(odo.x-wps[wp_i][0],odo.y-wps[wp_i][1])<3 or now-wp_t>300:
        wp_i=(wp_i+1)%len(wps); wp_t=now
        LOG.write(json.dumps({'ev':'wp','wp':wps[wp_i]})+'\n')
    cx,cy=wps[wp_i]
    if state=='follow':
        if front<0.3:
            outb=math.degrees(math.atan2(cy-odo.y,cx-odo.x))%360
            best=None;bs=-1e9
            for i in range(16):
                if L[i]<0.7: continue
