...
class Odo:
    def __init__(self):
        ...
        self.eL,self.eR=enc(); self.UPC=0.0015650
    def update(self):
        eL,eR=enc()
...
WPS=[(-38.5,9.0),(-37.3,5.5),(-30.0,5.5),(-24.0,6.0),(-17.0,4.5),(-10.0,4.0),(-10.0,12.0),(-15.0,18.0)]
wpi=0; wp_t=time.time()
def clean(v): return 3.0 if v<0 else v
state='run'; turn_tgt=0; last_tx=0; score=None
goal_latched=False; homing=False
scores=deque(); last_uturn=time.time()
while True:
  try:
    l=lidar()
    ...
    if rx and ('ATGOAL' in rx or rx.strip().startswith('GOALFOUND!')) and not homing:
        homing=True
        LOG.write(json.dumps({'ev':'homing','rx':rx})+'\n')
    try: score=float(s5)
    except: pass
    ...
    LOG.write(json.dumps({'t':round(now,2),'x':round(odo.x,3),'y':round(odo.y,3),
      'h':round(odo.h or -1,1),'l':l,'g':goal_flag,'d5':s5,'st':state,'wp':wpi})+'\n')
    if goal_flag!='0':
        if not goal_latched:
            goal_latched=True; LOG.write(json.dumps({'ev':'GOAL'})+'\n')
        stop(); wr('d0','GOALFOUND! A at goal, stopped. B come via range.'); time.sleep(0.7); continue
    if now-last_tx>2.0:
        wr('d0','A: exploring, you tail me. d5=%s'%(s5 or '?')); last_tx=now
    if homing:
        # chemotaxis uphill
        if state=='run' and score is not None and len(scores)>12 and now-last_uturn>10:
            if score<scores[0][1]-0.02:
                turn_tgt=((odo.h or 0)+180)%360; state='turn'; last_uturn=now
        tx,ty=odo.x,odo.y  # dummy
        bearing=None
    else:
        tx,ty=WPS[wpi]
        if math.hypot(tx-odo.x,ty-odo.y)<1.5 or now-wp_t>150:
            if wpi<len(WPS)-1: wpi+=1; wp_t=now
            LOG.write(json.dumps({'ev':'wp','wp':wpi})+'\n')
        bearing=math.degrees(math.atan2(ty-odo.y,tx-odo.x))%360
    if state=='run':
        if front<0.3:
            cands=[(i,L[i]) for i in range(16) if L[i]>0.7]
            if not cands: cands=[(max(range(16),key=lambda k:L[k]),3)]
            if bearing is not None:
                def sc(c):
                    i,r=c; ang=((odo.h or 0)+22.5*i)%360
                    dd=abs(((ang-bearing+180)%360)-180)
                    return -dd/45+r*0.4
                i=max(cands,key=sc)[0]
            else:
                pick=[c for c in cands if not (5<=c[0]<=11)] or cands
                i=random.choice(pick)[0]
            turn_tgt=((odo.h or 0)+22.5*i)%360; state='turn'
        else:
            u=0.0
            u+= 20*max(0,0.22-min(L[13],L[14],L[15]))/0.22
            u-= 20*max(0,0.22-min(L[1],L[2],L[3]))/0.22
            if bearing is not None and abs(u)<1:
                err=((bearing-(odo.h or 0)+180)%360)-180
                # steer gently toward bearing if roughly open that way
                u-=max(-8,min(8,err*0.15))  # err>0 -> need h increase -> motors(+,-) -> u negative? motors(base+u,base-u): u>0 turns h down
            base=100 if front>1.5 else (55 if front>0.7 else 32)
            if score is not None and score<0.5: base=min(base,45)
            if score is not None and score<0.35: base=min(base,22)
            if homing and score is not None and score>0.93: base=min(base,30)
    ...
    stop(); LOG.write(json.dumps({'err':str(e)})+'\n'); time.sleep(0.5)
