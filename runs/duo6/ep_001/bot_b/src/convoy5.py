...
while True:
  try:
    ...
    while hist and now-hist[0][0]>120: hist.popleft()
    if state=='follow' and now-last_hop>120 and len(hist)>60:
        xs=[p[1] for p in hist]; ys=[p[2] for p in hist]
        if math.hypot(max(xs)-min(xs),max(ys)-min(ys))<9:
    ...
    verylow = score is not None and score<0.35
    if state=='follow':
        if front<0.14:
            state='pivot'
        ...
        if front>0.35: state='follow'
        else: motors(40*side,-40*side)
    time.sleep(0.08)
