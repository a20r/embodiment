...
while k<N:
    ...
    if st and ('goal=0' not in st or 'here=0' not in st):
        log('!!! GOAL '+st); save()
        wr('d8',"R1: I AM ON THE GOAL NOW. Come to me: move so signal rises to max, then check your status shows here=1. Stay there.")
        break
    ...
    for w in path:
        ...
        while time.time()-t0<300:
            ...
            if time.time()-lastmsg>25:
                wr('d8',f"R1: waiting for you, sig {s_:.2f}. Move so it rises above 0.85, then I continue leading."); lastmsg=time.time()
            time.sleep(3)
        log(f"  follower sig {sig():.2f} after {time.time()-t0:.0f}s; stepping {w}")
        trav,reason=step(w)
        if trav>0.3:
            pos=nb(pos,w)
            if pos not in cells:
