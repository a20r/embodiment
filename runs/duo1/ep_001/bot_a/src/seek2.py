...
def turn_to_reading(ref, tol=6, timeout=20):
    ...
    while time.time()-t0<timeout:
        ...
        rob.motors(s,-s) if e>0 else rob.motors(-s,s)
        time.sleep(0.1)
    rob.motors(0,0); return False

def dive():
    """verify then drive toward goal until blocked."""
    turn_to_reading(0)
    L=rob.lidar()
    if not (L[0]>0.45 or L[0]<0):
        return "false"
    steps=0
    while True:
        ...
        turn_to_reading(0)
        L=rob.lidar()
        if 0<L[0]<0.3: return "blocked" if steps else "false"
        rob.motors(16,16)
        while True:
            L=rob.lidar()
            if (0<L[0]<0.3) or (0<L[1]<0.16) or (0<L[15]<0.16):
                rob.motors(0,0); break
            if abs(serr(bng(),0))>30: rob.motors(0,0); break
            time.sleep(0.05)
        steps+=1
        if rob.goal(): return "GOAL"
...
def follow(side, tmax, spd=18):
    ...
    while time.time()-t0<tmax:
        ...
        gd=L[gb]
        # door: open beam within 25deg of goal bearing, sustained
        if gd>0.6 and abs(serr(b, gb*22.5))<15:
            opens+=1
            if opens>=2:
                rob.motors(0,0)
                r=dive()
                print(f"  dive attempt -> {r} b={bng():.0f} t={time.time()-t0:.0f}", flush=True)
                if r=="GOAL": return "GOAL"
                opens=0
                if r=="blocked": return "moved"
                continue
        else: opens=0
        F=L[0]; D=L[side]
...
def main():
    t0=time.time(); side=12
    while time.time()-t0<3600:
        r=follow(side, 120)
        print(f"follow({side})->{r} b={bng():.0f} t={time.time()-t0:.0f}", flush=True)
        if r=="GOAL": print("GOAL!!!", flush=True); return
        if r=="tmax":
            side=16-side
            print("switch ->", side, flush=True)
