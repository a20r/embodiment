...
lasttx=0; steps=0; visited={}; stuckn=0; esc=1; homing=True; sig={}; TGT=(-2.8,2.85); prevd11=None; trend=0.0; lastabs=None
log('start')
while True:
    odo(); align(); odo()
    sc=ctl.scan(3); rec=log('scan',sc)
    if ('here=1' in (rec['d3'] or '')) or ('goal=1' in (rec['d3'] or '')):
        ctl.stop(); log('GOAL'); open('/bot/src/tx_msg.txt','w').write('A ON GOAL (here=1) at my (%.1f,%.1f). Stopped, waiting. Home in on rising d11 (signal). Tell me when you arrive.\n'%(x,y))
        while True:
            time.sleep(5); r=log('ONGOAL'); 
            if ('here=1' not in (r['d3'] or '')) and ('goal=1' not in (r['d3'] or '')): break
        continue
    if rec.get('rx') and 'GOAL AT' in rec['rx'].upper() and not homing:
        homing=True; log('HOMING-ON',None,rec['rx'])
    if homing and math.hypot(x-TGT[0],y-TGT[1])<0.45:
        ctl.stop(); log('AT-LANDMARK'); open('/bot/src/rx.log','a').write('SELF | ROUTE: N 1.2, W 0.3, N 0.3, W 0.3, N 0.3, W 0.4, N 0.25, W 0.95, N 0.55 | d11=0\n'); time.sleep(5); break
    try:
        cur=float(rec['d11']); sig[(round(x/CELL),round(y/CELL))]=cur
        trend = 0.0 if prevd11 is None else cur-prevd11; prevd11=cur
    except: pass
    ...
    for name,ang,dist in (('L',90,l),('F',0,f),('R',-90,r),('B',180,b)):
        ...
        if homing:
            dn=math.hypot(nx-TGT[0],ny-TGT[1]); dh=math.hypot(x-TGT[0],y-TGT[1])
            score=(dh-dn)*8 + (1.5 if cell not in visited else -1.5*visited[cell])
            tr = 0 if abs(trend)<0.02 else (1 if trend>0 else -1)
            absd=(h+ang)%360
            if lastabs is not None:
                if absd==lastabs: score += 3*tr
                elif absd==(lastabs+180)%360: score -= 3*tr
            if name=='F': score += 0.3
            if name=='B': score -= 1.0
            there=sig.get(cell)
            if there is not None: score += (there-float(rec["d11"]))*10
        opts.append((score,name,ang))
    if not opts: opts=[(0,'B',180)]
    opts.sort(reverse=True); sc_,name,ang=opts[0]
    if ang: ctl.turn(ang)
    log('turn'+name,None,'tr=%.2f opts=%s'%(trend,[(round(s,1),n) for s,n,a in opts]))
    visited[(round(x/CELL),round(y/CELL))]=visited.get((round(x/CELL),round(y/CELL)),0)+1
    lastabs=round(ctl.heading(3)/90)*90%360
    odo()
    ...
    stuckn = stuckn+1 if d<150 else 0
    if stuckn>=2:
        log('ESCAPE'); rio.wr('d7','-45'); rio.wr('d1','-45'); time.sleep(1.4); ctl.stop()
        ctl.turn(90*esc); esc=-esc; ctl.forward(0.3*S, v=40, stopfront=0.25); odo(); stuckn=0; continue
    if slip:
