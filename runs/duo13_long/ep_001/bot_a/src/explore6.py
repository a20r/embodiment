...
logf = open('/bot/src/explore6.log','a', buffering=1)
def log(*a):
...
ox=oy=0.0
try:
    _sv=json.load(open('/bot/src/explore5_state.json'))
    occ={tuple(map(int,k.split(','))):v for k,v in _sv['occ'].items()}
    log('resumed map, cells', len(occ))
except Exception:
    pass
x=y=0.0
try:
    _pp=json.load(open('/bot/src/last_pose.json'))
    ox=_pp['gx']; oy=_pp['gy']; log('pose offset',ox,oy)
except Exception:
    pass
h = r.heading() or 0.0
...
def scan_update():
    ...
    for i,d in enumerate(lid):
        ...
        ex=GX()+d*math.sin(az); ey=GY()+d*math.cos(az)
        if d<2.4:
            c=occ.setdefault(key(ex,ey),[0,0]); c[0]+=1
            mark_free(GX(),GY(),GX()+(d-0.08)*math.sin(az),GY()+(d-0.08)*math.cos(az))
        else:
            mark_free(GX(),GY(),GX()+2.3*math.sin(az),GY()+2.3*math.cos(az))
...
def radio():
    n=0
    while True:
        r.send(f'A PING {GX():.1f} {GY():.1f} {h:.0f} n={n}')
        n+=1
        t0=time.time()
...
try:
    for round_i in range(400):
        ...
        if st and (st[1] or st[2]):
            log('FLAG!', st, f'pose {GX():.2f},{GY():.2f} h={h:.0f}')
            json.dump({'x':GX(),'y':GY(),'h':h,'flags':st}, open('/memory/goal_found.json','w'))
            break
        update_pose()
        json.dump({'gx':GX(),'gy':GY(),'h':h}, open('/bot/src/last_pose.json','w'))
        scan_update()
        start=key(GX(),GY())
        if state(start)!=1:
        ...
        if round_i%5==0:
            ...
            json.dump({'x':GX(),'y':GY(),'h':h,'occ':{f'{a},{b}':v for (a,b),v in occ.items()}}, open('/bot/src/map5.json','w')); json.dump({'ox':ox,'oy':oy,'occ':{f'{a},{b}':v for (a,b),v in occ.items()}}, open('/bot/src/explore5_state.json','w'))
