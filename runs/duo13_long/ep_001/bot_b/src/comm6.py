...
PLAN=("B PLAN: search for goal. d3 flag goal=1 means goal visible, here=1 means standing on it. "
      "Whoever gets here=1: stay on goal and send 'X HERE' every 2s. "
      "Other robot homes on d11 (range) to arrive. Report goal/here in every ping.")
while True:
    try:
        n+=1
        a=rd("d9"); b=rd("d6")
        ...
        if n%12==0: wr("d8",PLAN)
        if n%40==0: wr("d8","B STATUS: no goal yet, exploring. A: report if you saw goal.")
