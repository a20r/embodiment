...
## EP3 FINAL WRAP (episode ended t~1787980600, ~100min)
- Goal NEVER fired (no goal=1 ever, either robot). Never co-located; best d5=0.95 (record; prior eps max 0.93).
- Ended with fin4.py running (basin explorer, tripwire 1.02, goal-hold); may keep acting after my shutdown.
- B's physical state at end: PARKED ROCKING NONSTOP at north end of its N-S corridor; its beacons repeat
  nav hints + "B pos 11 -1". B auto-freezes at d5>1.02. B's agent may or may not restart in EP4.
### EP4 CHECKLIST (do in order)
1. Read this file fully. cp /memory/src_backup/*.py /bot/src/.
2. RECALIBRATE speed via lidar slope (cal.py pattern; ep1 0.0028, ep2 0.0060, ep3 0.0171 per unit - may change again!).
   Patch drive.py v=K*speed; turn cap ~4-7 for ~20-35deg/s. Then patch script speeds accordingly.
3. Start bg2.py (goal watcher). Listen d4: B's beacon tells you its state. If B's agent alive, re-agree:
   "B PARK+ROCK NONSTOP" (it complies), you home.
4. Run fin3.py (strict d5 climb + tight-door sweeps, fs 0.115) until d5~0.94 core, then fin4.py
   (variance detector + basin explore). TRY: accept EAST tight door from 0.94 core even at 0.91-0.92 and
   explore 4-6 cells east before judging.
5. Tripwire: any d5>1.02: freeze 90s watching d6 for goal=1; broadcast STOP TEST. If goal=1: HOLD FOREVER +
   broadcast "A at_goal 1 GOAL STAY PUT" (B honors hold protocol).
6. Beware: self-kill via pgrep/pkill -f (PID-only kills); stuck detection (identical readings); nohup $! is
   sometimes shell PID not python (verify with ps).
### OPEN QUESTIONS
- Does goal exist as a place at all? 400+ distinct cells visited across eps, goal=0 always. Strongest remaining
  hypothesis: goal fires only when both robots co-located (or meet first, then flag appears). Rendezvous first.
- d5 semantics: zone-coupled sound level; same-cell value unknown (>0.95). Radio range ~ d5>0.25; msgs may queue.
- Wall-math trick works: myClr + wallThick(0.15) + itsClr = k*0.52 locates B's side.
1787980616.8 RX: B pos 11 -1 goal 0
1787980617.2 f4 95 2,10 d5=0.75
1787980618.6 RX: B pos 11 -1 goal 0
