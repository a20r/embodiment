...
while True:
    ...
    r.events=[]
    if now-last_tx>2:
        r.tx.write('A wallrunning to find route into your pocket. STAY PARKED SPINNING. Report d5+pos every 5s. My d5 %s'%r.d5.last)
        last_tx=now
    if goal_seen:
        ...
        while True:
            ...
            time.sleep(0.2)
    try: dv=float(r.d5.last)
    except: dv=0.0
    if dv>1.1:
        L('TRIPWIRE %.2f freeze'%dv); r.wheels(0,0)
        tf=time.time()
        while time.time()-tf<70:
            r.update()
            for m in r.msgs: L('RX:',m)
            r.msgs=[]
            for e in r.events:
    ...
    for ax in order:
        ...
        tr,reason=d.forward(0.5,target_h=ax,front_stop=0.23,speed=30)
        step+=1
        if tr>0.28:
