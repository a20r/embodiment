...
fn main() {
    evlog("RIDGE", "greedy crest walker begin");
    let mut cur = sample_d11();
    let dirs = [0i64, 45, -45, 90, -90, 135, -135, 180];
    let mut stall = 0;
    let leg: u64 = 1200;
    loop {
        if at_goal() {
            evlog("GOAL", "flag fired - holding");
            loop { stop(); radio("A1 ATGOAL holding - R1 come!");
                   if let Some(m) = read_str("d10") { evlog("RX", &m); }
                   thread::sleep(Duration::from_millis(700)); }
        }
        if let Some(m) = read_str("d10") { evlog("RX", &m); }
        let lid = read_lidar();
        let fm = (3..=8).map(|i| lid[i]).fold(f64::INFINITY, f64::min);
        if fm < 0.3 {
            evlog("ESC", &format!("front={:.2}", fm));
            run(-20.0, -20.0, 1000);
        ...
        let mut moved = false;
        for &d in dirs.iter() {
            spin_to(norm(h0 + d as f64));
            run(20.0, 20.0, leg);
            let v = sample_d11();
            evlog("TRY", &format!("rel={} d11={:.4} cur={:.4}", d, v, cur));
            if !v.is_nan() && !cur.is_nan() && v > cur + 0.002 {
                cur = v; moved = true; stall = 0;
                break;
            }
        }
        if !moved {
            ...
            run(20.0, 20.0, leg * 3);
            let v = sample_d11();
            evlog("LONG", &format!("d11={:.4}", v));
            if !v.is_nan() && !cur.is_nan() && v > cur { cur = v; stall = 0; }
            if stall >= 2 {
                radio(&format!("A1 on d11 shelf {:.3}, searching crest", cur));
                spin_to(norm(heading().unwrap_or(0.0) + 90.0));
                stall = 0;
            }
        }
        radio(&format!("A1 ridge d11={:.3}", cur));
    }
}
