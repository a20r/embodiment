...
def deep():
    ...
    for p in s.split(';'):
        ...
        try:
            ...
            if r>0.05 and e<-0.9 and r>best: best=r
        except: pass
    return best
...
def lmed():
    ...
    for p in s.split(';'):
        ...
        try:
            r,e,a=map(float,p.split(','))
            if r>0.02 and 0.3<e<1.6: vals.append(r)
        except: pass
    vals.sort()
    return vals[len(vals)//2] if vals else -1
...
while time.time()-t0<300:
    ...
    if 'goal=1' in s3 or 'here=1' in s3:
        logf.write(f"!!! FLAG {time.time():.0f} {s3} deep={deep():.2f} med={lmed():.2f}\n")
        if 'goal=1' in s3:
            w('d1',0); w('d7',0)
            while True:
                broadcast(f"GOALFOUND\n"); time.sleep(2)
    dp=deep(); md=lmed()
    logf.write(f"GO {time.time():.0f} h={H:.0f} deep={dp:.2f} med={md:.2f} d11={read('d11')} {s3}\n")
    ...
    w('d1',0); w('d7',0); time.sleep(0.05)
    if mvd<60:
        stall+=1
        logf.write(f"GO STALL {stall}\n")
        t2=time.time()
        while time.time()-t2<0.6:
            w('d1',-55); w('d7',-55); time.sleep(0.15); poll()
        w('d1',0); w('d7',0)
        if stall>=2:
            TARGET=(TARGET+(30*stall))%360; stall=0
            logf.write(f"GO target->{TARGET}\n")
    else:
        stall=0
logf.write("GO end\n")
