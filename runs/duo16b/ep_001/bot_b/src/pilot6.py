...
R='/dev/robot/'
READ_PORTS=['d3','d4','d6','d9','d11','d0','d5']
...
def do_explore(n=25):
    ...
    for it in range(n):
        ...
        if front is not None and front>0.45:
            ...
            # arc turn: outer wheel only, keeps moving forward, minimal drift
            if left>right:
                # want CCW (heading-): right wheel fwd: d7
                fdm=os.open(R+'d7',os.O_WRONLY|os.O_NONBLOCK); os.write(fdm,b"55\n"); os.close(fdm)
                res["turn"]="arcCCW"
            else:
                fdm=os.open(R+'d1',os.O_WRONLY|os.O_NONBLOCK); os.write(fdm,b"55\n"); os.close(fdm)
                res["turn"]="arcCW"
            tA=time.time()
            while time.time()-tA<0.55:
                if check_goal(): break
            stop()
        results.append(res)
        if it%2==0:
...
def radio_thread():
    while running:
        try:
            ...
            t0=time.time()
            while running and time.time()-t0<0.4:
