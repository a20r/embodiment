...
def clearance_grid(pts, nbins=9):
    ...
    for r,c1,c2 in pts:
        if c1<-0.3: continue  # skip steep-down rows (floor)
...
def do_rot(deg,cmd=50):
    ...
    tgt=h0+deg; s=1 if deg>0 else -1
    write_motors(-cmd*s, cmd*s)
    t0=time.time(); 
    while time.time()-t0<abs(deg)/(1.74*cmd)*2+1.5:
        h=geth()
        if h is not None and abs(norm180(tgt,h))<3: break
        if check_goal(): stop(); log("GOAL during rot"); break
    stop(); time.sleep(0.2)
    h1=geth()
    return {"rot":round(norm180(h0,h1),1) if h1 is not None else None}
