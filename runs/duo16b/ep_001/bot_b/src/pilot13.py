...
def do_g3(sec=420):
    ...
    while time.time()<t_end:
        ...
        while time.time()-t0<14.0:
            ...
            if goal_seen(): break
        stop(); time.sleep(0.2)
        ln3=lastline(read1('d3',0.06) or '')
        if ln3:
            try:
                dd=dict(kv.split('=') for kv in ln3.split() if '=' in kv)
                if dd.get('goal')=='1': log("!!! GOAL=1 !!!"); stop(); do_tx("A ATGOAL ATGOAL"); json.dump(out,open('/memory/g3log.json','w')); return {"GOAL":True,"sweeps":len(out)}
                if dd.get('here')=='1': log("!!! HERE=1 !!!"); do_tx("A HERE1"); json.dump(out,open('/memory/g3log.json','w')); return {"HERE":True,"sweeps":len(out)}
            except Exception: pass
